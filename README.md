...
============================================================
  ...
  ➜  Dashboard: https://xxxxxxxxxx.cloudfront.net?api=https://xxxxxxxxxx.execute-api.us-east-1.amazonaws.com/prod/&region=...
  ➜  Login:     admin@devops.local / <auto-generated-password>
  ➜  Agent Space Console: https://us-east-1.console.aws.amazon.com/aidevops/home?region=us-east-1#/agent-spaces/...

------------------------------------------------------------
  NEXT STEPS

  1. Open the dashboard link above and sign in

  2. Go to the 'Configuration' tab to find:
     • MCP server endpoint and OAuth credentials
     • S3 bucket ARNs for Agent Space
     • Webhook configuration

  3. In the Agent Space console:
     • Register the MCP server (use values from dashboard)
     • Generate a webhook URL and HMAC secret
     • Paste them into the dashboard webhook config

  4. Go to the 'Networking Scenarios' tab
     • Click 'Break' on any scenario to trigger an investigation
